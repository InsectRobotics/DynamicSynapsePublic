...
class DynamicSynapseArray:
    def __init__(self, NumberOfSynapses = 5, CW = 100, tauWV = 40, aWV = 100, rWV = 5000, scale=10,  \
                ...
                DampingUpdateRate = 0.0000003/100/350 , WeightersCentreUpdateCompensate =0, MaxDamping = 10*7000):
        ...
        self.WeighterInDendrite =  self.WeighteAmountPerSynapse* self.NumberOfSynapses[1] - self.WeightersLast.sum(axis=1,keepdims=True)
        ...
        self.EquivalentVolume = (1+(2*self.WeightersCentre-(self.WeighterInDendriteConcentration+self.WeightersLast))/((self.WeighterInDendriteConcentration+self.WeightersLast)-self.WeightersCentre))
    ...
    def Derivative (self, state=None , inputs=None, Parameters=None):
        ...
        if inputs is not None:
            WeighterInDendriteConcentration=inputs
        else:
            WeighterInDendriteConcentration=self.WeighterInDendriteConcentration
        ...
        EquivalentVolume = (1+(2*WeightersCentre-(WeighterInDendriteConcentration+WeightersLast))/((WeighterInDendriteConcentration+WeightersLast)-WeightersCentre))
...
#        DWeighters =  WeighterVarRatesLast * WeighterInDendriteConcentration * WeightersLast/scale
        DWeighterVarRates = (  (tauWV*(WeighterInDendriteConcentration - WeightersLast/EquivalentVolume ) \
...
#        +aWV * np.sign(WeighterVarRatesLast)*np.power(np.abs(WeighterVarRatesLast), 0.5))/ rWV  - WeighterVarDamping*WeighterVarRatesLast ) /  tauWV  /self.scale
    ...
    def StepSynapseDynamics(self, dt, ModulatorAmount, Compensate='Constant'):
        if isinstance(ModulatorAmount,(list, tuple,np.ndarray)):
            ...
            if len(ModulatorAmount.shape)<2:
                self.ModulatorAmount=ModulatorAmount[0]
                ModulatorAmount=ModulatorAmount.reshape(np.append(ModulatorAmount.shape,1))
                
        else:
            self.ModulatorAmount=ModulatorAmount
        self.Parameters = [self.CW, self.tauWV, self.aWV , self.rWV, self.scale, self.WeightersCentre,self.WeighterVarDamping]
        self.Weighters, self.WeighterVarRates = rk4(dt, np.array([self.WeightersLast, self.WeighterVarRatesLast]), self.WeighterInDendriteConcentration, self.Parameters, self.Derivative)
        ...
        self.WeighterInDendriteConcentration = self.WeighterInDendrite/self.NumberOfSynapses[1]
#        assert np.all(np.greater(self.WeighterInDendriteConcentration, 0)), "Negative WeighterInDendriteConcentration"
#        assert np.all(np.greater(self.Weighters, 0)), "Negative Weighters"
...
#        self.WeightersCentre += (self.Weighters-self.WeightersCentre)*ModulatorAmount *self.WeightersCentreUpdateRate*dt #0.000015##0.00002
    ...
    def InitRecording(self, lenth, SampleStep = 1):
        ...
        self.Trace = {'Weighters':np.zeros(np.append(lenth,self.NumberOfSynapses).ravel()), \
                        ...
                        'WeighterInDendriteConcentration' :  np.zeros(np.append(np.append(lenth,self.NumberOfSynapses[0]),1).ravel()), \
...
#            print(Temp)
        ...
        if self.RecordingInPointer>= self.RecordingLenth:
            if N_Append == 0:
                ...
                for key in self.Trace:
                    if key == 'WeighterInDendriteConcentration' or  key == 'ModulatorAmount':
                        
                        self.Trace[key]=np.append(self.Trace[key],np.zeros(np.append(np.append(N_Append,self.NumberOfSynapses[0]),1).ravel()), axis=0)
                    else:
...
#    plt.legend(figure1lines2, labels)
    ...
    AxonConcentrationLines=plt.plot(TracetInS, TraceWeighterInDendriteConcentration,linewidth= linewidth)
...
if __name__=="__main__":
    ...
    if InintialDS:
        NumberOfNeuron=2
        NumberOfSynapses = 6# N =3 tauWV =50; #N = 6 tauWV = 25
...
#                ADSA.DampingUpdateRate= DampingUpdateRateCache
            ...
            ADSA.StepSynapseDynamics( SimulationTimeInterval,1, Compensate='Dynamic0')
