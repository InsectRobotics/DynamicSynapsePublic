...
#        DWeighters = (WeighterVarRatesLast  * ( WeighterInAxonConcentration + WeightersLast/EquivalentVolume +np.sign(WeighterVarRatesLast)*(WeighterInAxonConcentration - WeightersLast/EquivalentVolume)) /2  + CW*(WeighterInAxonConcentration - WeightersLast/EquivalentVolume))/self.scale    
#        DWeighters = EquivalentVolume *  (WeighterVarRatesLast  * ( WeighterInAxonConcentration + WeightersLast/EquivalentVolume +np.sign(WeighterVarRatesLast)*(WeighterInAxonConcentration - WeightersLast/EquivalentVolume)) /2 )/self.scale   
#        DWeighters =  WeighterVarRatesLast  * ( WeighterInAxonConcentration + WeightersLast +np.sign(WeighterVarRatesLast)*(WeighterInAxonConcentration - WeightersLast)) /2  /self.scale    
#        DWeighters = EquivalentVolume *  WeighterVarRatesLast  /self.scale
#        DWeighters =  WeighterVarRatesLast  /self.scale

        DWeighters =  (WeighterVarRatesLast  *0.5*(1-np.exp( -100*( WeighterInAxonConcentration + WeightersLast/EquivalentVolume +np.sign(WeighterVarRatesLast)*(WeighterInAxonConcentration - WeightersLast/EquivalentVolume)) /2 )))/self.scale   
#        DWeighters =  WeighterVarRatesLast * WeighterInAxonConcentration * WeightersLast/scale
        DWeighterVarRates = (  (tauWV*(WeighterInAxonConcentration - WeightersLast/EquivalentVolume ) \
...
#        ax.plot( Wis,(b*h*s+np.sqrt(b**2 * h**2 * s**2 - 4*a*h*s*(d*h*s-d*s*w+h*w-w**2)))/(2*a*h*s) , lw=2, label='v-nullcline 3' ) 
        ...
        ax.plot( Wis,np.zeros(Wis.shape) -Wis+d, lw=2, label='w-nullcline' ) 
        ax.set_ylim (ylim)
...
if __name__=="__main__":
    ...
    if InintialDS:
        ...
        Weighters= 0.6#np.random.rand(NumberOfNeuron,NumberOfSynapses) #0.5*np.ones(NumberOfSynapses) +0.001*np.random.rand(NumberOfSynapses)  #
...
#        SimulationTimeInterval = 30
    ...
    #    scale=1
 #0.4 0.05       
        WeightersCentral = 0.7#(NumberOfSynapses) #* np.random.rand(NumberOfNeuron,NumberOfSynapses) # 0.6 * np.random.rand(NumberOfNeuron,NumberOfSynapses) #np.array([4, 1, 1, 1, 1])#np.ones(NumberOfSynapses)*0.4 + 0.3 * np.random.rand(NumberOfSynapses) #np.ones(NumberOfSynapses)/2 + [0.8, 0.1,0.1, 0.1, 0]  #[0.2, 0.1, 0]
...
#        figure1,figure2,figure3,figure4,figure5, figure6, figure7,figure8,figure9,ax = plot(TimOfRecording, Traces, path='/media/archive2T/chitianqilin/SimulationResult/DynamicSynapse/Plots/', savePlots=True, linewidth= 0.2) #path=
    ...
    if PlotPhasePotrait :
        ViewScale=40
        PhasePotraitfig, PhasePotraitax = ADSA.PlotPhasePortrait( xlim=[0,1], ylim=[-0.0001, 0.0001], fig=None, ax=None, inputs=[ WeighteAmountPerSynapse-WeightersCentral , WeightersCentral], Parameters=None)
 ...
 #       PhasePotraitfig, PhasePotraitax = ADSA.PlotPhasePortrait( xlim=[0.0,1.5], ylim=[-0.000005*ViewScale, 0.000005*ViewScale], fig=None, ax=None, inputs=[1, WeightersCentral], Parameters=None)
        ...
        lc = LineCollection(segments,colors=color) # cmap=plt.get_cmap('copper'),
        PhasePotraitax.add_collection(lc)
        #%%
        plt.figure()
        plt.plot(Tracet,ADSA.Trace['Weighters'][:,0,0])
